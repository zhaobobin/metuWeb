...
export default class UserLogin extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      remember:
        Storage.get(ENV.storage.remenber) !== null
          ? Storage.get(ENV.storage.remenber)
          : true,
      ...
      <div className={styles.sign}>
        <div className={styles.form}>
          ...
          <p className={styles.loginType}>
            <a>
              {loginType === 'psd' ? (
                <span onClick={() => this.changeLoginType('sms')}>
                  短信快速登录
                </span>
              ) : (
                <span onClick={() => this.changeLoginType('psd')}>
                  账号密码登录
                </span>
              )}
            </a>
          ...
          <Form onSubmit={this.submit}>
              ...
              <FormItem>
                {getFieldDecorator('smscode', {
                  ...
                  <InputSmscode
                    type="login"
                    mobile={
                      Validator.hasErrors(getFieldsError(['mobile']))
                        ? ''
            ...
            {loginType === 'psd' ? (
              <FormItem style={{ height: '40px' }}>
                ...
                <a className={styles.forgotPsd} onClick={this.toPsdReset}>
                  忘记密码
                </a>
                <br />
              </FormItem>
        ...
        <div className={styles.foot}>
          <p className={styles.loginChange}>
            ...
            <a className={styles.r}>
              {loginType === 'psd' ? (
                <span onClick={() => this.changeLoginType('scan')}>
                  切换到二维码登录
                </span>
              ) : (
                <span onClick={() => this.changeLoginType('psd')}>
